...
service /assets on new http:Listener(8080) {
    ...
    resource function post [string tag]/components(@http:Payload Component comp) returns http:Created|http:NotFound|http:Conflict|http:BadRequest {
        ...
        lock {
            ...
            if comp.serial.clone().toString().length() > 0 {
                cid = newId.trim().length() > 0 ? newId.trim() : comp.serial.clone().toString();
            }
            else {
            ...
            if asset.components.hasKey(cid) {
                return <http:Conflict>{body: {message: "component with this id already exists"}};
            }
